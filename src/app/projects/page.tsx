        ...
        >
          <Section href="/projects" text="My Projects & Contributions">
            ...
            >
              ...
              >
                <SelectTrigger className="mb-8 flex items-center gap-2 border px-4 py-2 text-sm font-medium">
    ...
    >
        ...
        {hosted_url && !hosted_url.includes("github.com") ? (
          <>
            ...
            <Image
              ...
              priority
            />
            <h3 className="absolute bottom-0 left-0 z-10 rounded-tr-2xl bg-background text-muted-foreground px-2 py-1 text-xl">
              {name}
            </h3>
          </>
        ) : (
          <div className="flex items-center justify-center h-full w-full bg-background relative">
            <div className="absolute inset-0 bg-[url('/assets/common/noise-bg.webp')] bg-repeat bg-[length:128px] opacity-10 rounded-2xl" />
            <h3 className="relative text-2xl font-bold text-muted-foreground z-10">
              {name}
            </h3>
          </div>
        )}
      ...
      <FlipCardBack className="relative overflow-auto rounded-2xl p-6 bg-black">
        ...
        <div className="relative z-10">
              ...
              {technologies.map((tech, idx) => {
                if (typeof tech === "string") {
                  return (
                    <MotionLi
                      key={idx}
                      variants={fadeInUp}
                      className="rounded-full border px-3 py-1 text-xs border-foreground bg-background text-foreground"
                    >
                      {tech}
                    </MotionLi>
                  );
                ...
                return (
                  <MotionLi
                    key={idx}
                    variants={fadeInUp}
                    className="flex items-center gap-2 rounded-full border px-3 py-1 text-xs border-foreground bg-background text-foreground"
                  >
          ...
          <div className="mt-10 flex gap-5 text-foreground">
